...
@section('body')
    ...
    <div id="main">
        ...
        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            ...
            <input id="name" name="name" type="text" placeholder="e.g. Fire Safety" value="" onkeyup="generateStub()" @if($errors->has('name')) class="error">
            <span class="error">{{ $errors->first('name') }}</span> @else > @endif

            <label>Project stub<em>(Used for URLs)</em></label>
            <input id="stub" name="stub" type="text" placeholder="e.g. firesafety" @if($errors->has('stub')) class="error">
            <span class="error">{{ $errors->first('stub') }}</span> @else > @endif

            <label>Current version</label>
            <input name="current_version" type="text" placeholder="e.g. Version 1" @if($errors->has('current_version')) class="error">
            <span class="error">{{ $errors->first('current_version') }}</span> @else > @endif

            <label>Project status</label>
            <input name="status" type="text" placeholder="e.g. In development, Launched" @if($errors->has('status')) class="error">
            <span class="error">{{ $errors->first('status') }}</span> @else > @endif

            <hr/>

            <label>Authoring Tool</label>
            <input name="authoring_tool" type="text" placeholder="e.g. Adapt, Storyline, Lectora" @if($errors->has('authoring_tool')) class="error">
            ...
            <input name="lms_specification" type="text" placeholder="e.g. SCORM 1.2, SCORM 2004" @if($errors->has('lms_specification')) class="error">
            <span class="error">{{ $errors->first('lms_specification') }}</span> @else > @endif

            <hr/>

            <label>Project manager</label>
            <input name="project_manager" type="text" placeholder="Start typing a name" @if($errors->has('project_manager')) class="error">
            <span class="error">{{ $errors->first('project_manager') }}</span> @else > @endif

            <label>Lead developer</label>
            <input name="lead_developer" type="text" placeholder="Start typing a name" @if($errors->has('lead_developer')) class="error">
            <span class="error">{{ $errors->first('lead_developer') }}</span> @else > @endif

            <label>Lead designer</label>
            <input name="lead_designer" type="text" placeholder="Start typing a name" @if($errors->has('lead_designer')) class="error">
            <span class="error">{{ $errors->first('lead_designer') }}</span> @else > @endif

            <label>Instructional designer</label>
            <input name="instructional_designer" type="text" placeholder="Start typing a name" @if($errors->has('instructional_designer')) class="error">
